...
import com.supermarket.archives.InsertStockFrame;
import com.supermarket.archives.UpdateDepotFrame;
import com.supermarket.archives.UpdateStockFrame;
import com.supermarket.bean.Stock;
import com.supermarket.dao.StockDao;
import com.supermarket.jbutton.ButtonEditor;
import com.supermarket.jbutton.ButtonRenderer;
...
public class JButtonTablePanel extends JPanel {
	...
	public JButtonTablePanel() {
		...
		dm.setDataVector(new Object[][] { { "", "", "", "", "", "", "", "" } },
				new Object[] { "是否入库", "编号", "客户", "订单号", "交货日期", "商品名", "金额",
						"数量" });
		dm.removeRow(0);
		table = new JTable(dm);
		...
		insertButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				InsertStockFrame frame = new InsertStockFrame();
		...
		deleteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				JOptionPane.showMessageDialog(getParent(), "数据删除成功！", "信息提示框",
				...
				if (condition.equals("货品名称")) { // 如果用户选择按货品名称进行搜索
					...
					for (int i = 0; i < list.size(); i++) { // 循环遍历查询结果
						...
						if (id <= 0) { // 如果该订单的货品在入库表中不存在
							dm.addRow(new Object[] { "入库", stock.getId(),
									stock.getsName(), stock.getOrderId(),
									stock.getConsignmentDate(),
									stock.getBaleName(), stock.getMoney(),
									stock.getCount() }); // 向表格中添加数据
